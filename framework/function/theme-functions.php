...
function register_css_styles() {
	if (!is_admin()) {
		...
		wp_register_style('theme-rtl', get_template_directory_uri() . '/css/rtl.css', array(), '1.0.0', 'screen');
...
function register_js_scripts() {
	if (!is_admin()) {
		...
		wp_register_script('libs', get_template_directory_uri() . '/js/jsLibraries.min.js', array(), '1.0.0', false);
		
		wp_register_script('jLibs', get_template_directory_uri() . '/js/jqueryLibraries.min.js', array('jquery'), '1.0.0', true);
		
		
		// wp_register_script('less', get_template_directory_uri() . '/js/less.min.js', array(), '1.3.3', false);
...
function cmsms_theme_google_font($fonts) {
	$protocol = is_ssl() ? 'https' : 'http';
	
	
	wp_enqueue_style('cmsms-google-fonts', $protocol . '://fonts.googleapis.com/css?family=' . $fonts);
}
...
function cmsms_composer_icons() {
	$url = file_get_contents(CMSMS_ADMIN . '/inc/fonts/config.json');
...
function cmsms_write_styles($styles) {
	$upload_dir = wp_upload_dir();
	...
	$file = trailingslashit($style_dir) . CMSMS_SHORTNAME . '.css';
...
function sidebarDefaultText() {
	...
	'<aside id="archives" class="widget widget_archive">' . "\n" . 
		'<h3 class="widgettitle">' . __('Archives', 'cmsmasters') . '</h3>' . "\n" . 
		'<ul>' . "\n";
	...
	'<aside id="meta" class="widget widget_meta">' . "\n" . 
		'<h3 class="widgettitle">' . __('Meta', 'cmsmasters') . '</h3>' . "\n" . 
		'<ul>' . "\n\t";
...
function cmsms_theme_bg_styles() {
	...
	$cmsms_def_bg_img = explode('|', $cmsms_option[CMSMS_SHORTNAME . '_bg_img']);
	...
	if ( 
		...
		!is_archive() && 
		!is_search() && 
		$cmsms_option[CMSMS_SHORTNAME . '_theme_layout'] == 'boxed' 
	) {
		...
		$cmsms_bg_img_str = get_post_meta($post->ID, 'cmsms_bg_img', true);
		...
		$cmsms_bg_size = get_post_meta($post->ID, 'cmsms_bg_size', true);
	}
...
<style type=\"text/css\">";
	...
	body {
		...
		background-image : " . (($cmsms_bg_img_enable == 'true' && is_array($cmsms_bg_img)) ? 'url(' . ((is_numeric($cmsms_bg_img[0])) ? array_shift(wp_get_attachment_image_src((int) $cmsms_bg_img[0], 'full')) : $cmsms_bg_img[1]) . ')' : 'none') . ";
		...
		background-size : " . (($cmsms_bg_img_enable == 'true') ? $cmsms_bg_size : 'auto') . ";
	}";
		} else {
			echo "
	body {
		background-color : " . $cmsms_option[CMSMS_SHORTNAME . '_bg_col'] . ";
		background-image : " . (($cmsms_option[CMSMS_SHORTNAME . '_bg_img_enable'] && !empty($cmsms_option[CMSMS_SHORTNAME . '_bg_img'])) ? 'url(' . ((is_numeric($cmsms_def_bg_img[0])) ? array_shift(wp_get_attachment_image_src((int) $cmsms_def_bg_img[0], 'full')) : $cmsms_def_bg_img[1]) . ')' : 'none') . ";
		...
		background-size : " . (($cmsms_option[CMSMS_SHORTNAME . '_bg_img_enable']) ? $cmsms_option[CMSMS_SHORTNAME . '_bg_size'] : 'auto') . ";
	}";
		}
...
add_action('wp_head', 'cmsms_theme_footer_styles');
