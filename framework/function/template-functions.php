...
function cmsms_logo() {
	...
	if ($cmsms_option[CMSMS_SHORTNAME . '_logo_type'] == 'text') {
		...
		if ($cmsms_option[CMSMS_SHORTNAME . '_logo_subtitle'] != '') {
			...
			$blog_descr = (get_bloginfo('description')) ? get_bloginfo('description') : __('Default Logo Subtitle', 'cmsmasters');
		}
		...
		if ($cmsms_option[CMSMS_SHORTNAME . '_logo_url'] == '') {
			echo '<a href="' . home_url() . '/" title="' . get_bloginfo('name') . '" class="logo">' . "\n\t" . 
				...
				'<img class="logo_retina" src="' . get_template_directory_uri() . '/img/logo_retina.png" alt="' . get_bloginfo('name') . '" width="172" height="70" />' . "\r" . 
			'</a>' . "\n";
...
";
			...
			if (is_numeric($logo_img[0])) {
				$logo = wp_get_attachment_image_src((int) $logo_img[0], 'full');
			}
			
			
			$logo_width = ((is_numeric($logo_img[0])) ? (int) $logo[1] : '172');
			
			$logo_height = ((is_numeric($logo_img[0])) ? (int) $logo[2] : '70');
			
			
			if ($logo_height >= $header_mid_height) {
				$logo_style_width = (int) ($header_mid_height * ($logo_width / $logo_height));
			} else {
				$logo_style_width = $logo_width;
			}
...
";
			...
			echo '<a href="' . home_url() . '/" title="' . get_bloginfo('name') . '" class="logo">' . "\n\t" . 
				'<img src="' . 
				((is_numeric($logo_img[0])) ? array_shift(wp_get_attachment_image_src((int) $logo_img[0], 'full')) : $logo_img[1]) . 
				'" alt="' . get_bloginfo('name') . '" />' . "\r";
			
			
			if ($cmsms_option[CMSMS_SHORTNAME . '_logo_url_retina'] != '') {
				if (is_numeric($logo_img_retina[0])) {
					$logo_retina = wp_get_attachment_image_src((int) $logo_img_retina[0], 'full');
				}
				
				$logo_retina_width = ((is_numeric($logo_img_retina[0])) ? ((int) $logo_retina[1] / 2) : '172');
				
				$logo_retina_height = ((is_numeric($logo_img_retina[0])) ? ((int) $logo_retina[2] / 2) : '70');
				
				
				echo "
<style type=\"text/css\">
	.header_mid_inner .logo .logo_retina {
		width : {$logo_retina_width}px;
	}
</style>
";
				...
				((is_numeric($logo_img_retina[0])) ? array_shift($logo_retina) : $logo_img_retina[1]) . 
				'" alt="' . get_bloginfo('name') . '" width="' . 
				((is_numeric($logo_img_retina[0])) ? ((int) $logo_retina[0] / 2) : '172') . 
				'" height="' . 
				((is_numeric($logo_img_retina[0])) ? ((int) $logo_retina[1] / 2) : '70') . 
				'" />' . "\r";
			} else {
				echo '<img class="logo_retina" src="' . 
				((is_numeric($logo_img[0])) ? array_shift(wp_get_attachment_image_src((int) $logo_img[0], 'full')) : $logo_img[1]) . 
...
function cmsms_footer_logo() {
	...
	if ($cmsms_option[CMSMS_SHORTNAME . '_footer_logo_type'] == 'text') {
		if ($cmsms_option[CMSMS_SHORTNAME . '_logo_title'] != '') {
			$blog_title = $cmsms_option[CMSMS_SHORTNAME . '_logo_title'];
		} else {
			$blog_title = (get_bloginfo('name')) ? get_bloginfo('name') : CMSMS_FULLNAME;
		...
		if ($cmsms_option[CMSMS_SHORTNAME . '_footer_logo_url'] == '') {
			echo '<a href="' . home_url() . '/" title="' . get_bloginfo('name') . '" class="logo">' . "\n\t" . 
				'<img src="' . get_template_directory_uri() . '/img/logo_footer.png" alt="' . get_bloginfo('name') . '" />' . "\n\t" . 
				'<img class="footer_logo_retina" src="' . get_template_directory_uri() . '/img/logo_footer_retina.png" alt="' . get_bloginfo('name') . '" width="172" height="70" />' . "\r" . 
			'</a>' . "\n";
...
";
			...
			$logo_img = explode('|', $cmsms_option[CMSMS_SHORTNAME . '_footer_logo_url']);
			
			$logo_img_retina = explode('|', $cmsms_option[CMSMS_SHORTNAME . '_footer_logo_url_retina']);
...
function cmsms_favicon() {
	...
	if ($cmsms_option[CMSMS_SHORTNAME . '_favicon']) {
		if (is_numeric($favicon_img[0])) { 
			echo '<link rel="shortcut icon" href="' . array_shift(wp_get_attachment_image_src((int) $favicon_img[0], 'full')) . '" type="image/x-icon" />';
		} else {
	...
	if ($cmsms_heading != 'disabled') {
		...
		echo "<style type=\"text/css\">";
			if ($cmsms_heading_bg_img_enable == 'true' && $cmsms_heading_bg_img != '') {
				echo ".headline_outer {
					background-image:url(" . ((is_numeric($options_img[0])) ? array_shift(wp_get_attachment_image_src((int) $options_img[0], 'full')) : $options_img[0]) . ");
