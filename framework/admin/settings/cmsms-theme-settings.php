...
function cmsms_get_settings() {
	...
	case CMSMS_PAGE_BASENAME . '-color':
		...
		$cmsms_settings_page_title = __('Theme Color Schemes', 'cmsmasters');
	...
	case CMSMS_PAGE_BASENAME . '-single':
		...
		$cmsms_settings_page_title = __('Theme Single Posts Settings', 'cmsmasters');
	...
	case CMSMS_PAGE_BASENAME . '-demo':
		...
		$cmsms_settings_page_title = __('Theme Settings Import/Export', 'cmsmasters');
...
function cmsms_form_field_fn($args = array()) {
	...
	case 'number':
		...
		echo '<input class="small-text cmsms-spinner-field' . $field_class . '" type="text" id="' . $id . '" name="' . $cmsms_option_name . '[' . $id . ']" value="' . $options[$id] . '"' . (($min != '') ? ' min="' . $min . '"' : '') . (($max != '') ? ' max="' . $max . '"' : '') . (($step != '') ? ' step="' . $step . '"' : '') . ' />' . 
		(($desc != '') ? ' &nbsp; ' . '<span class="description">' . $desc . '</span>' : '');
	...
	case 'social':
		...
		'<div class="icon_management">' . 
			'<p>' . 
				'<input class="icon_upload_image all-options" type="hidden" id="new_icon_name" value="" />' . 
				'<span id="new_icon_icon"></span>' . 
				'<input id="' . $id . '_button" class="icon_choose_button button" type="button" value="' . __('Choose icon', 'cmsmasters') . '" />' . 
			'</p>' . 
			...
			'</span>' . 
			'<span class="cl"></span>' . 
			'<input class="button button-primary" type="button" id="add_icon" value="' . __('Add Icon', 'cmsmasters') . '" />' . 
			'<input class="button button-primary" type="button" id="edit_icon" value="' . __('Save Icon', 'cmsmasters') . '" />' . 
			'<input class="button" type="button" id="cancel_icon" value="' . __('Cancel changes', 'cmsmasters') . '" />' . 
			'<ul>';
			...
			if (isset($options[$id]) && is_array($options[$id])) {
				foreach($options[$id] as $icon) {
					...
					echo '<li>' . 
						...
						'</div>' . 
						'<a href="#" class="icon_del admin-icon-remove" title="' . __('Remove', 'cmsmasters') . '"></a> ' . 
...
function cmsms_settings_page_fn() {
	...
	'<p class="submit-top">' . 
		(($current_tab != 'import' && $current_tab != 'export') ? '<input name="submit-top" type="submit" class="button button-primary button-large" value="' . __('Save Changes', 'cmsmasters') . '" />' : '') . 
	'</p>';
	...
	echo '<p class="submit">' . 
				(($current_tab != 'import' && $current_tab != 'export') ? '<input name="submit" type="submit" class="button button-primary button-large" value="' . __('Save Changes', 'cmsmasters') . '" />' : (($current_tab == 'import') ? '<input name="import" type="button" class="button button-primary button-large cmsms-demo-import" value="' . __('Import Settings', 'cmsmasters') . '" />' : '')) . 
			'</p>' . 
		'</form>' . 
	'</div>';
}
